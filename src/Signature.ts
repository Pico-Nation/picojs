...
import { constructElliptic, PublicKey } from './picojs-key-conversions';

/** Represents/stores a Signature and provides easy conversion for use with `elliptic` lib */
export class Signature {
    constructor(private signature: Key, private ec: EC) {}

    /** Instantiate Signature from an PICOIO-format Signature */
    ...
    public static fromElliptic(ellipticSig: EC.Signature, keyType: KeyType, ec?: EC): Signature {
        ...
        let picoioRecoveryParam;
        if (keyType === KeyType.k1 || keyType === KeyType.r1) {
            picoioRecoveryParam = ellipticSig.recoveryParam + 27;
            if (ellipticSig.recoveryParam <= 3) {
                picoioRecoveryParam += 4;
            }
        } else if (keyType === KeyType.wa) {
            picoioRecoveryParam = ellipticSig.recoveryParam;
        }
        const sigData = new Uint8Array([picoioRecoveryParam].concat(r, s));
        if (!ec) {
    ...
    /** Export Signature as PICOIO-format Signature */
