...
import { ecc as eccMigration } from '../picojs-ecc-migration';

import { PrivateKey } from '../picojs-key-conversions';
...
describe('ecc Migration', () => {
    ...
    it('verifies `privateToPublic` function is consistent between ecc objects', () => {
        console.warn = jest.fn();
        const eccPublicKey = ecc.privateToPublic(privateKeys[0], 'PICO');
        const eccMigrationPublicKey = eccMigration.privateToPublic(privateKeys[0], 'PICO');
    ...
    it('verifies `isValidPublic` function is consistent between ecc objects', () => {
        console.warn = jest.fn();
        const eccValid = ecc.isValidPublic(legacyPublicKeys[0], 'PICO');
        const eccMigrationValid = eccMigration.isValidPublic(legacyPublicKeys[0], 'PICO');
        expect(console.warn).toHaveBeenCalledWith('Argument `pubkey_prefix` is deprecated, ' +
            'keys prefixed with PUB_K1_/PUB_R1_/PUB_WA_ going forward');
    ...
    it('verifies `isValidPublic` function is consistent during an error', () => {
        console.warn = jest.fn();
        const eccValid = ecc.isValidPublic('publickey', 'PICO');
        const eccMigrationValid = eccMigration.isValidPublic('publickey', 'PICO');
        expect(console.warn).toHaveBeenCalledWith('Argument `pubkey_prefix` is deprecated, ' +
            'keys prefixed with PUB_K1_/PUB_R1_/PUB_WA_ going forward');
        expect(eccValid).toEqual(eccMigrationValid);
