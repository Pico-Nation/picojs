...
const { Signature, PrivateKey, PublicKey, sha256 } = require('../picojs-key-conversions');
const {
    JsSignatureProvider,
} = require('../picojs-jssig');
const { KeyType } = require('../picojs-numeric');
const { SignatureProviderArgs } = require('../picojs-api-interfaces');
...
describe('JsSignatureProvider', () => {
    ...
    it('(NOTE: sigs are different): ensure elliptic does what picojs-ecc used to do', () => {
    ...
    it('ensure elliptic verifies picojs-ecc\'s Sigs', () => {
        const ellipticEc = new ec('secp256k1');
        for (let idx=0; idx<privateKeys.length; idx++) {
            const KPriv = privateKeys[idx];
            const KPrivElliptic = PrivateKey.fromString(KPriv).toElliptic();
    ...
    it('ensure picojs verifies picojs-ecc\'s Sigs', () => {
        for (let idx=0; idx<privateKeys.length; idx++) {
            const KPriv = privateKeys[idx];
            ...
            const eccHashedString = Buffer.from(ecc.sha256(dataAsString), 'hex');
            const picojsHashedStringAsBuffer = Buffer.from(sha256(dataAsString), 'hex');
            expect(eccHashedString).toEqual(picojsHashedStringAsBuffer);

            const eccSig = ecc.sign(dataAsString, KPriv, 'utf8');

            const picojsSig = Signature.fromString(eccSig);
            const recoveredKPub = ecc.recover(eccSig, dataAsString, 'utf8');
            const picojsRecoveredKPub = picojsSig.recover(dataAsString, true, 'utf8');

            expect(picojsRecoveredKPub.toLegacyString()).toEqual(recoveredKPub);
            expect(picojsRecoveredKPub.toString()).toEqual(k1FormatPublicKeys[idx]);

            const picojsValid = picojsSig.verify(dataAsString, picojsRecoveredKPub, true, 'utf8');
            expect(picojsValid).toEqual(true);
        }
    });

    it('ensure ecc verifies picojs\'s Sigs', () => {
        for (let idx=0; idx<privateKeys.length; idx++) {
            const KPriv = privateKeys[idx];
            const privateKey = PrivateKey.fromString(KPriv);
            ...
            const picojsSig = privateKey.sign(picojsHashedStringAsBuffer, false, 'utf8');
            const picojsSigAsString = picojsSig.toString();

            const recoveredKPub = ecc.recover(picojsSigAsString, dataAsString, 'utf8');
            const picojsRecoveredKPub = picojsSig.recover(dataAsString, true, 'utf8');
            expect(picojsRecoveredKPub.toLegacyString()).toEqual(recoveredKPub);
            expect(picojsRecoveredKPub.toString()).toEqual(k1FormatPublicKeys[idx]);

            const eccValid = ecc.verify(picojsSigAsString, dataAsString, recoveredKPub, 'utf8');
            expect(eccValid).toEqual(true);
        }
    });
});
