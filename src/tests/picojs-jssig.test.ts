...
import {generateKeyPair, PrivateKey, PublicKey, sha256, Signature} from '../picojs-key-conversions';
import {digestFromSerializedData, JsSignatureProvider} from '../picojs-jssig';
import {KeyType} from '../picojs-numeric';
import {SignatureProviderArgs} from '../picojs-api-interfaces';
...
describe('JsSignatureProvider', () => {
    ...
    const legacyPublicKeys = [
        'PICO7tgwU6E7pAUQJgqEJt66Yi8cWvanTUW8ZfBjeXeJBQvhTU9ypi',
        'PICO8VaY5CiTexYqgQZyPTJkc3qvWuZUi12QrZL9ssjqW2es6aQk2F',
        'PICO7VGhqctkKprW1VUj19DZZiiZLX3YcJqUJCuEcahJmUCw3wJEMu',
    ];
    ...
    describe('secp256k1 elliptic', () => {
        ...
        it('confirm elliptic conversion functions are actually reciprocal', async () => {
            ...
            const picoSig = Signature.fromElliptic(ellipticSig, KeyType.k1);
            expect(sig).toEqual(picoSig);
        });
        ...
        it('ensure public key functions are actual inverses of each other', async () => {
            const picoioPubKey = PublicKey.fromString(k1FormatPublicKeys[0]);
            const ellipticPubKey = picoioPubKey.toElliptic();
            const finalPicoioKeyAsK1String = PublicKey.fromElliptic(ellipticPubKey, KeyType.k1).toString();
            expect(finalPicoioKeyAsK1String).toEqual(k1FormatPublicKeys[0]);
        });

        it('verify that PUB_K1_ and Legacy pub formats are consistent', () => {
            const picoioLegacyPubKey = legacyPublicKeys[0];
            const ellipticPubKey = PublicKey.fromString(picoioLegacyPubKey).toElliptic();
        ...
        it('ensure private key functions are actual inverses of each other', async () => {
            ...
            const privPicoioKey = PrivateKey.fromString(priv);
            const privEllipticKey = privPicoioKey.toElliptic();
            const finalPicoioKeyAsString = PrivateKey.fromElliptic(privEllipticKey, KeyType.k1).toString();
            expect(privPicoioKey.toString()).toEqual(finalPicoioKeyAsString);
        });
    ...
    describe('p256 elliptic', () => {
        ...
        it('confirm elliptic conversion functions are actually reciprocal', async () => {
            ...
            const sig: Signature = Signature.fromString(signOutput.signatures[0]);
            const ellipticSig: ec.Signature = sig.toElliptic();
            const picoSig = Signature.fromElliptic(ellipticSig, KeyType.r1);
            expect(sig).toEqual(picoSig);
        });
        ...
        it('ensure public key functions using p256 format are actual inverses of each other', async () => {
            const picoioPubKey = PublicKey.fromString(r1FormatPublicKeys[0]);
            const ellipticPubKey = picoioPubKey.toElliptic();
            const finalPicoioKeyAsR1String = PublicKey.fromElliptic(ellipticPubKey, KeyType.r1).toString();
            expect(finalPicoioKeyAsR1String).toEqual(r1FormatPublicKeys[0]);
        });
        ...
        it('ensure private key functions using p256 format are actual inverses of each other', async () => {
            ...
            const privPicoioKey = PrivateKey.fromString(priv);
            const privEllipticKey = privPicoioKey.toElliptic();
            const finalPicoioKeyAsString = PrivateKey.fromElliptic(privEllipticKey, KeyType.r1).toString();
            expect(privPicoioKey.toString()).toEqual(finalPicoioKeyAsString);
        });
